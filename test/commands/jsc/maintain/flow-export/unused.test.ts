...
import JscMaintainExportUnusedFlows from '../../../../../src/commands/jsc/maintain/flow-export/unused.js';
...
describe('jsc maintain flow-export unused', () => {
  ...
  it('shows no table output and logs success when no unused versions are found', async () => {
    // Act
    await JscMaintainExportUnusedFlows.run(['--target-org', $$.testTargetOrg.username]);

    // Assert
    expect(sfCommandStubs.table.callCount).to.equal(0);
    expect(sfCommandStubs.logSuccess.args.flat()).to.deep.equal([messages.getMessage('success.no-unused-flows-found')]);
  });

  it('shows table output when unused versions are found', async () => {
    // Arrange
    $$.unusedFlows = parseMockResult('unused-flows.json');
    ...
    expect(sfCommandStubs.table.args.flat()[0].data.length).to.equal(5);
    expect(sfCommandStubs.logSuccess.callCount).to.equal(0);
  });

  it('shows summarized table output when unused versions are found with --concise flag', async () => {
    // Arrange
    $$.unusedFlows = parseMockResult('unused-flows.json');

    // Act
    await JscMaintainExportUnusedFlows.run(['--target-org', $$.testTargetOrg.username, '--concise']);
    ...
    expect(sfCommandStubs.table.args.flat()[0]).to.deep.equal({
      data: [
        ...
        { FlowDefinitionName: 'Test_Flow_3', Versions: 2 },
  ...
  it('exports explicit flow versions to package.xml with --concise flag', async () => {
    ...
    // Act
    const outputDir = `${$$.outputDir}/unused`;
    await JscMaintainExportUnusedFlows.run([
      '--target-org',
    ...
    expect(packageXml.Package.types[0].members.length).to.equal(5);
  });
});
